...
class LSTM_CTE_Model(nn.Module):
    ...
    def build(self, x, mode, eps=1e-6):
        ...
        context_de_output = self.decoder_no_answer(no_answer_state, context_dec, context_tar, cat=torch.max(pure_answer_output, dim = 1, keepdim=True)[0])
    ...
    def forward(self, x):
        loss, _,_,_, closs, _ = self.build(x, mode= 'train')
        return loss, closs

    def predict(self, x):
        # loss, answer_only_state, no_answer_state, pure_answer_output,_ = self.build(x)
        loss, answer_only_state, no_answer_state, pure_answer_output, _, sampled_words = self.build(x, mode= 'train')
        de_words_answer = []
        if answer_only_state is not None:
            de_words_answer = self.decoder_answer.generate(answer_only_state)
        de_words_context = self.decoder_no_answer.generate(no_answer_state, cat = torch.max(pure_answer_output, dim = 1, keepdim=True)[0])
